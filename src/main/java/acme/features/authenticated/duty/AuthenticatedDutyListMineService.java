...
public class AuthenticatedDutyListMineService implements AbstractListService<Authenticated, Duty> {
	...
	public boolean authorise(final Request<Duty> request) {
		assert request != null;
		Job job = this.repository.findJobByDescriptorId(request.getModel().getInteger("id"));
	...
	public Collection<Duty> findMany(final Request<Duty> request) {
		assert request != null;
		return this.repository.findManyByDescriptorId(request.getModel().getInteger("id"));
	}
